...
class MRIDataset(Dataset):
    ...
    Args:
        train (bool): if True, applies random flip augmentation along spatial axes
    """

    def __init__(self, data_root=None, label_json_path=None, seq_len=None, split=None, train=False):
        self.data_root = data_root or cfg.DATA_ROOT
        self.seq_len   = seq_len or cfg.SEQ_LEN
        self.train     = train  # NEW: controls augmentation
    ...
    def __getitem__(self, idx):
        ...
        # NEW: training-time augmentation — random flips along D, H, W axes
        # x_seq shape: (T, 1, D, H, W) -> axes 2, 3, 4 are spatial
        if self.train:
            for axis in [2, 3, 4]:
                if torch.rand(1).item() > 0.5:
                    x_seq = torch.flip(x_seq, dims=[axis])
